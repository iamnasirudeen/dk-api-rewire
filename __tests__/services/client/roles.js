...
const getUrl = pathname => url.format({ // eslint-disable-line
...
const createSuperAdminData = async () => {
  role = 'Super Admin';
  // Create the user
  await utils.createTestUser(superAdminEmail, password, role, 'Individual', superAdminUsername, [true, true, true]);
  // log the user in
  superToken = await utils.clientAuthenticate(superAdminEmail, password);
  headers.Authorization = superToken;
  // Create a new resource
  superData = await makeClientRequest('/roles', 'POST', superData);
};
...
describe('User Roles', () => {
  ...
  it('Super Admin Can Add/Manage Role', async () => {
    // role = 'Super Admin';
    // // Create the user
    // await utils.createTestUser(superAdminEmail, password, role, 'Individual', superAdminUsername, [true, true, true]);
    // // log the user in
    // const token = await utils.clientAuthenticate(superAdminEmail, password);
  ...
  it('Admin Can Not Add/Manage Role', () => {
    role = 'Admin';
    // Create the user
    return utils.createTestUser(adminEmail, password, role, 'Individual', adminUsername, [false, false, false]).then(() => {
      // log the user in
      return utils.clientAuthenticate(adminEmail, password).then(token => {
        headers.Authorization = token;
        // Create a new resource
        return makeClientRequest('/roles', 'POST', testData).catch(res => {
          expect(res.error.code).toBe(403);
          expect(res.error.message).toBe('Admin cannot create Roles');
          expect(res.error.name).toBe('Forbidden');
          let path = `/roles/${superData._id}`;
          superData.name = '_MODIFIED_';
          return makeClientRequest(path, 'PUT', superData).catch(res => {
            expect(res.error.code).toBe(403);
            expect(res.error.message).toBe('Admin cannot update Roles');
            expect(res.error.name).toBe('Forbidden');
            return makeClientRequest(path, 'DELETE').catch(res => {
              expect(res.error.code).toBe(403);
              expect(res.error.message).toBe('Admin cannot remove Roles');
              expect(res.error.name).toBe('Forbidden');
            });
          });
        });
